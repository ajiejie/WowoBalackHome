...
StartScene.isTime = 1;  --用来标识返回按钮按了几次

function StartScene:init()
	--键盘操作
	self:setKeypadEnabled(true)
	self:addNodeEventListener(cc.KEYPAD_EVENT,function (event)
		if event.key == "back" then  --按返回键
			if StartScene.isTime == 1 then
				self.message = MessageBox.new();
				self.message:setPosition(0, 0);
		...
		elseif event.key =="menu" then  --按菜单键 进入设置界面
	...
	self._sp:setPosition(display.right-150, display.top-150)
	self._sp:setScale(2)
	...
	self._snow:setPosition(display.cx, display.top)
    ...
    self.startButton:setPosition(480, 400)
    ...
    self.helpButton:setPosition(480, 200)
...
function StartScene:help()
	...
	self._help:setPosition(display.cx, display.cy-200)
	...
	btn:setPosition(self._help:getContentSize().width/2, 100)
